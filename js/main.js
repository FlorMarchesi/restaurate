...
function cargarProductosCarrito(array) {
  ...
  array.forEach((productoCarrito) => {
    ...
    carritoModal.innerHTML += `<div class="card mb-3 cardProducto" style="max-width: 540px;" id ="productoCarrito${productoCarrito.id}">
  <div class="row g-0">
    <div class="col-4">
      <img class="img-fluid rounded-start" src="assets/${productoCarrito.image}" alt="${productoCarrito.nombre}">
    </div>
    <div class="col-8">
      <div class="card-body">
        <h5 class="card-title">${productoCarrito.nombre}</h5>
        <p class="card-text">${productoCarrito.precio} U$D</p>
        <button class= "btn btnEliminarCarrito" id="botonEliminar${productoCarrito.id}"><i class="fas fa-trash-alt"></i></button>
      </div>
    </div>
  </div>
</div>`
...
function agregarAlCarrito(opcion) {
  ...
  productoAgregado == undefined ? (
    ...
    Toastify({
      ...
      gravity: "bottom",
      style: {
        background: "linear-gradient(to right, #619b8aff, #a1c181ff)",
      }
    }).showToast()) :
    ...
    Toastify({
      ...
      gravity: "bottom",
      style: {
        background: "linear-gradient(to right, #fe7f2dff, #fcca46ff)",
      }
    }).showToast()
}
...
function compraTotal(array) {
  ...
  precioTotal.innerHTML = `El total del carrito es <span>${acumulador} U$D</span>`
}
// CARRITO
// FUNCIONES
